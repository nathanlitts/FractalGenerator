...
public class MandelbrotCalc implements Calculator {
		...
		for (int row = 0; row < height; row++) 
            for (int col = 0; col < width; col++) 
                if (plane[row][col].getValue() == -2) 
                    calculateValue(plane[row][col], maxIter, exponent);
	}
     ...
     */
    public void calculateValue(ComplexNumber c, int maxIter, int exponent) {
